...
const ChatBoard: React.FC = () => {
  ...
  const normalizeText = (text: string) => {
    // Convert full-width numbers and letters (Ａ-Ｚ, ａ-ｚ, ０-９) and symbols like '＝＞' to half-width
  ...
  const processMessage = (text: string) => {
    ...
    // Detect dice rolls: (\d+)[dD](\d+)
    const diceRegex = /(\d+)[dD](\d+)/gi;
    let hasDice = false;

    // We replace each match with the processed string
    ...
    // Detect attack commands: (\d+)\s*=>\s*(\d+)
    ...
    resultText = resultText.replace(attackRegex, (match, atkStr, defStr) => {
      ...
      isSystem = true; // Mark as system if attack calculation is triggered
